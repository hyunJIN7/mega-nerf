...
class Embedding(nn.Module): #Positional Encoding
...
class NeRF(nn.Module):
    def __init__(self, pos_xyz_dim: int, pos_dir_dim: int, layers: int, skip_layers: List[int], layer_dim: int,
        ...
        self.embedding_xyz = Embedding(pos_xyz_dim) # pos_xyz_dim : positional encoding의 L 인듯
        in_channels_xyz = xyz_dim + xyz_dim * pos_xyz_dim * 2  #원래 xyz positional encoding 거치고 나서 60 인데 여기선 75

        self.skip_layers = skip_layers # 5번째에 γ(x) 추가되는 layer

        xyz_encodings = []

        # xyz encoding layers TODO : nerf fully-connected network architecture , density 나오기 전까지 network
        ...
        self.xyz_encodings = nn.ModuleList(xyz_encodings)  #density 나오기 전까지 network
        # density part
        if pos_dir_dim > 0:  # 차수 올리는 파트 , pos_dir_dim : positional encoding의 L 인듯
        ...
        if appearance_dim > 0:  #TODO : 이건 뭐냐?
    ...
    def forward(self, x: torch.Tensor, sigma_only: bool = False,
        ...
        expected = self.xyz_dim \
                   ...
                   + (0 if (sigma_only or self.embedding_a is None) else 1)  #embedding_a : embedding(190,48)
